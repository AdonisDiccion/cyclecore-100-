  ...
  return (
    ...
    <TotalStocks/>
      ...
      <div className="bg-white shadow-lg py-2 px-4 border-b flex items-center justify-between">
        ...
        <button className="p-2 hover:text-yellow-500" onClick={() => navigate("/dashboard/admin/stocks/add")}><GiReturnArrow/></button>
      </div>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-3">
            ...
            <div className="text-gray-600 max-w-xs text-center mx-auto">
              <h2>Please specify the desired month and year to generate a report containing the data for newly added stock of each product.</h2>
            </div>
          </div>
          ):(
            <div>
            {monthlyData && Object.keys(monthlyData).length ? (
                    ...
                    <tr key={productName} className="border-b">
                      <td className="p-2">
                        <div className="flex gap-1">
                          <div><img src={productData.photoUrl} alt=""  className="w-20"/></div>
                          <div>{productName}</div>
                        </div>
                      </td>
                      <td className="p-2">{productData.value} added stocks</td>
                      <td className="p-2">{moment(productName.createdAt).format("MMMM Do YYYY, h:mm:ss")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="max-w-xs text-center mx-auto font-bold text-2xl tracking-wide space-y-3">
                ...
                <h2>No stocks added on <span className="text-red-500">{`${moment().month(selectedMonth).format("MMMM")} ${selectedYear}`}</span></h2>
              </div>
            )}
          </div>
        )}
